...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect unt.aimable source path. Please configure manually in settings.');
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# unt.aimable Framework Environment Variables',
          '# Managed by unt.aimable UI',
          '',
