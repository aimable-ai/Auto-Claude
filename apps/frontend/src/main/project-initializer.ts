...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (fileExists) {
    ...
    writeFileSync(gitignorePath, '# unt.aimable data directory\n' + entriesToAdd.join('\n') + '\n', 'utf-8');
  }
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  // Check git status - unt.aimable requires git for worktree-based builds
  const gitStatus = checkGitStatus(projectPath);
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    debug('Git check failed', { gitStatus });
    return {
      success: false,
      error: gitStatus.error || 'Git repository required. unt.aimable uses git worktrees for isolated builds.'
    };
  }
 ...
 * not an installation. This allows unt.aimable to be used to develop itself.
 */
